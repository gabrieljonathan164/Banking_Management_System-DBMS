...
/***What if we want to find the customer who has opened an account on or before 2020-07-15***/


select CustName, CustAddress,dateregistered  from Customer
where dateregistered <= '2020-07-15'; 

/****What if we want to find customer with Account balance of more than 5000 
who has a savings account and having minimum balance more than 5000***/
...
/****Write a query to find customer having a savings account with interest rate less than 0.5****/
...
/******Write a query to find Account having monthly fee more than 10 and 
ATM withdrawal cap of more than 500*****/

SELECT CheckingAccount.Monthly_fee, CheckingAccount.ATM_withdrawalcap, Account.AccountName
from CheckingAccount
JOIN Account ON Account.AccountID = CheckingAccount.AccountID
WHERE monthly_fee > 10 and atm_withdrawalcap > 500;


/****What if we want to find out name of the customer Account 
whose transaction was declined for an amount of more than 50 $ ****/
select Transactions.TransDate, Transactions.TransAmount,Account.AccountName
FROM Transactions 
